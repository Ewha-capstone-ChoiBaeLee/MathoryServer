...
namespace SharedData.Models
{
    public class Prompt
    {
        static void Main(string[] args)
        {
            ...
            string friend;
            List<string> remainingFriends = SelectingFriends(friends, out friend);
            string clock = clocks[new Random().Next(clocks.Count)];
            string location = locations[new Random().Next(locations.Count)];
            string goal = goals[new Random().Next(goals.Count)];
            ...
            if (goal == "수업")
            {
                ...
                goal = $"{subjectWhy[new Random().Next(subjectWhy.Count)]} {subject[new Random().Next(subject.Count)]} {goal} {subjectHow[new Random().Next(subjectHow.Count)]}";
            ...
            else if (goal == "파티")
            {
                ...
                goal = $"{party[new Random().Next(party.Count)]} {goal} {partyHow[new Random().Next(partyHow.Count)]}";
            }
            else if (goal == "아이템")
            {
                string itemOwner = friend;
                List<string> item = new List<string> { "책을", "마법 반지를", "안경을" };
                List<string> itemHow = new List<string> { "찾기", "제작하기", "고치기" };
                goal = $"{itemOwner}의 {item[new Random().Next(item.Count)]} {itemHow[new Random().Next(itemHow.Count)]}";
                location = locations[new Random().Next(locations.Count)];
            }
            else if (goal == "선물을 구매")
            {
                remainingFriends.Remove(friend);
                string giftOwner = remainingFriends[new Random().Next(remainingFriends.Count)];
                List<string> giftWhy = new List<string> { "의 생일", "와 화해하기 위해", "의 최근의 일을 위로하기 위해", "가 평소 좋아하는 물건을 발견해서", "가 좋아하는", "의 떨어진 성적을 위로하기 위해" };
                goal = $"{giftOwner}{giftWhy[new Random().Next(giftWhy.Count)]} {goal}";
                location = "마법 마을";
            }
            else if (goal == "동아리")
            {
                ...
                goal = $"{club[new Random().Next(club.Count)]} {goal} {clubHow[new Random().Next(clubHow.Count)]}";
                location = locations[new Random().Next(locations.Count)];
            }
            else if (goal == "간식")
            {
                List<string> snackHow = new List<string> { "먹기", "요리하기", "선물하기" };
                goal = $"{goal} {snackHow[new Random().Next(snackHow.Count)]}";
                location = locations[new Random().Next(locations.Count)];
            }
            else if (goal == "운동")
            {
                ...
                goal = $"{sport[new Random().Next(sport.Count)]}{sportHow[new Random().Next(sportHow.Count)]}";
                location = "학교 운동장";
                clock = "낮";
            }
        ...
        static List<string> SelectingFriends(List<string> friends, out string friend)
        {
            Console.WriteLine("원하는 친구를 고르세요: 루카스[0], 마크[1], 소피아[2], 리나[3], 제시카[4]");
            string[] selectFriends = Console.ReadLine().Split(' ');
            List<string> selectedFriends = new List<string>();

            foreach (string idxStr in selectFriends)
            {
                int idx = int.Parse(idxStr);
                selectedFriends.Add(friends[idx]);
            }

            List<string> remainingFriends = new List<string>();
            for (int i = 0; i < friends.Count; i++)
            {
                if (!selectFriends.Contains(i.ToString()))
                {
                    remainingFriends.Add(friends[i]);
                }
            }

            friend = "주인공과 " + string.Join("와 ", selectedFriends) + "가 ";
